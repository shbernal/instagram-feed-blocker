...
`release/instagram-feed-blocker-source-<version>.zip`. Release builds must
archive the tag, not `HEAD`; the script warns when it archives `HEAD` with a
dirty working tree.

`git archive` emits exactly the tracked tree at that ref, so `node_modules/`,
`dist/`, `dist-firefox/`, `release/`, `.e2e/` and untracked scratch files are
excluded by construction rather than by an exclude list that has to be
maintained. The archive includes `pnpm-lock.yaml`, which the reviewer build
depends on.
...
> Build environment: Ubuntu 24.04, Node.js 24.
>
...
management honor the pin and switch to 11.20.0 inside this repository, so the
