...
// says it should. Instagram's markup is brittle by constraint and the content
...
Loads dist/ in a headed Chromium, opens an Instagram URL, and prints a JSON
...
storage contents, the popup's rendered controls, and the data-igfb-* attributes
...
Signed out, Instagram redirects to its login page and the content script
correctly matches nothing. The snapshot says which page it landed on so that is
...
Environment
  ...
  INSPECT_PROFILE_DIR             reuse a profile directory instead of a
                                  ...
                                  .e2e/instagram-real-profile to inspect a
                                  signed-in session
...
const startUrl = process.argv[2] ?? 'https://www.instagram.com/'
...
try {
  ...
  // The managed attributes are the evidence that blocking took hold. Read off
  // the live page rather than restated here, so a new section appears with no
  // change to this file.
  snapshot.page = await attempt(() =>
    page.evaluate(() => {
      const all = Array.from(document.querySelectorAll('*')).flatMap(element =>
        Array.from(element.attributes)
          .filter(attribute => attribute.name.startsWith('data-igfb-'))
          .map(attribute => ({
