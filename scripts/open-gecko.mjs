...
window open until you close it. The default URL is the Instagram home page.

The profile is thrown away and recreated on every launch, under
node_modules/.tmp/gecko-profiles/dev-<binary>, so it holds no Instagram session.
Sign in to Instagram in the window if a check needs one.
...
// Namespaced by binary so Firefox and Zen never share one.
