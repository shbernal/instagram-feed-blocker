...
describe('toggle command', () => {
  it('asks the active Instagram tab to toggle blocking', async () => {
    await loadServiceWorker()
    const chromeMock = getChromeMock()
    withActiveTab({ id: 9, url: INSTAGRAM_TAB_URL })
  ...
  it('leaves a non-Instagram tab alone', async () => {
    await loadServiceWorker()
    const chromeMock = getChromeMock()
  ...
  it('does nothing when the active tab has no id', async () => {
    await loadServiceWorker()
    const chromeMock = getChromeMock()
    withActiveTab({ url: INSTAGRAM_TAB_URL })
