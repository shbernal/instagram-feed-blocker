...
describe('parseShortcut', () => {
  it('parses the default Chrome binding', () => {
    expect(parseShortcut('Ctrl+Shift+9')).toEqual({
      ...noModifiers,
      ctrlKey: true,
      shiftKey: true,
      code: 'Digit9',
  ...
  it('maps Command to the Meta key on macOS bindings', () => {
    expect(parseShortcut('Command+Shift+9')).toEqual({
      ...noModifiers,
      shiftKey: true,
      metaKey: true,
      code: 'Digit9',
  ...
  it('parses glyph-only macOS bindings', () => {
    expect(parseShortcut('⌘⇧9')).toEqual({
      ...noModifiers,
      shiftKey: true,
      metaKey: true,
      code: 'Digit9',
...
describe('matchesShortcut', () => {
  const shortcut = parseShortcut('Ctrl+Shift+9')

  it('matches the exact modifier set and key code', () => {
    expect(
      matchesShortcut(
        keydown({ code: 'Digit9', ctrlKey: true, shiftKey: true }),
        shortcut,
      ),
    ).toBe(true)
  })
  ...
  it('rejects extra or missing modifiers', () => {
    expect(
      matchesShortcut(keydown({ code: 'Digit9', ctrlKey: true }), shortcut),
    ).toBe(false)
    expect(
      matchesShortcut(
        keydown({
